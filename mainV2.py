...
parser = argparse.ArgumentParser(description='COMparador PREcios VEhículos')
...
# /MARCA/MODELO/, fregto=DESDE AÑO, fregfrom=HASTA AÑO, version0=VERSION


url =  "https://www.autoscout24.es/lst/%s/%s?sort=standard&desc=0&offer=U&ustate=N" % (marca, modelo) + "%2CU" + "&cy=E&atype=C&size=20&fregto=%d&fregfrom=%d&version0=%s" % (any_f, any_i, ver) 
    ...
    for j in range (1,10):
        ...
        if status_code == 200:
            ...
            for i in range(len(precios)):  
                precio=precios[i].text #precio
                #contralamos si tiene versión o no (no todos tienen)
                filas=t2[i].find_all('h2')
                mod=t2[i].find_all('h2')[0].text #modelo
                km=tags[i].find_all('li')[0].text #kilometraje
                Anyo=tags[i].find_all('li')[1].text #Ayo
                ...
                if (len(filas))==2:
                   ...
                   DatosCoches=[mod,ver,precio,km,Anyo,KWCV,Automatico,Elec]
                else:
                   DatosCoches=[mod,"n/a",precio,km,Anyo,KWCV,Automatico,Elec]
...
ListaCoches=[]
Cabecera=["Modelo","Versión","Precio","Kilometraje","Año","Motor","Tipo"]
